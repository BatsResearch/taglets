...
from zsl_kg_lite.gnn.mean_agg import MeanAggregator
from zsl_kg_lite.gnn.combine import Combine

class GCNConv(nn.Module):
    def __init__(self, features, adj_lists, device, options):
        """The GCNConv class encoder for generating class representations.
        The GCNConv class encoder uses mean aggregator and combine
        ...
        for i in options['gnn'][::-1]:
            agg = MeanAggregator(current_enc, device, num_sample=i['num_sample'],
                                 sample_nodes=i['sample_nodes'],
                                 dropout=i['dropout'], self_loop=i['agg_self_loop'])

            enc = Combine(current_enc, input_dim, i['output_dim'],
                          adj_lists=adj_lists, aggregator=agg, device=device,
                          self_concat=i['combine_self_concat'], leaky_relu=i['leaky_relu'],
                          relu=i['relu'])
