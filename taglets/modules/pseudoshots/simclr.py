...
class ResNet(nn.Module):
    def __init__(self, layers, width_multiplier, sk_ratio, adapt_pool_size=None):
        super().__init__()
        ...
        if adapt_pool_size:
          ops.append(nn.AdaptiveAvgPool2d(adapt_pool_size))
          channels_in = ops[-2].channels_out
        else:
          channels_in = ops[-1].channels_out
...
def get_resnet(depth=50, width_multiplier=1, sk_ratio=0, adapt_pool_size=None):  # sk_ratio=0.0625 is recommended
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = ResNet(layers, width_multiplier, sk_ratio, adapt_pool_size)
    return resnet 
