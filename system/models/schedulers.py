...
def make_scheduler(optimizer, config):
    ...
    if config.SCHEDULER == "cosine":
        ...
        lr_scheduler = LambdaLR(
            optimizer, lr_lambda=lambda epoch : config.WARMUP_LR / config.LR if epoch == 0 else 1
        )
    else:
        lr_scheduler = scheduler.StepLR(optimizer, step_size=config.STEP_SIZE, gamma=0.1)    
    return lr_scheduler
