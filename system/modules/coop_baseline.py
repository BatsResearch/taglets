...
class CoopBaseline(object):
    ...
    def _train_epoch(self, loss, total_loss, train_loader, accum_iter, epoch):
        ...
        for img, _, _, label, img_path in tqdm(train_loader):
            ...
            #log.info(f"Predictions: {idx_preds}")
            #log.info(f"Seen to real dictionary: {self.seen_to_real}")

            # real_preds = [self.seen_to_real[i.item()] for i in idx_preds]
            ...
            # log.info(f"Preds: {real_preds}")
            # log.info(f"Labels: {label}")
            # log.info(f"Correct: {torch.sum(label == torch.tensor(real_preds))}")

            loss = self.loss_func(logits, idx_preds)
