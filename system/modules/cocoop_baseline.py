...
class ExtendedTextPrefixModel(nn.Module):
    def __init__(self, initial_prefix, 
                 text_encoder, classes, image_feat_dim, prefix_dim,
                 temperature=0.07, device='cpu'):
        super(ExtendedTextPrefixModel, self).__init__()
        self.device = device
        ...
        self.meta_net = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(image_feat_dim, image_feat_dim // 16)),
            ("relu", nn.ReLU(inplace=True)),
            ("linear2", nn.Linear(image_feat_dim // 16, prefix_dim))
        ]))

        self.meta_net_dtype = list(self.meta_net.parameters())[0].dtype

    def forward(self, image_feats):
        bias = self.meta_net(image_feats.to(self.meta_net_dtype))
        bias = bias.unsqueeze(1)
        prefix_shifted = self.prefix + bias
        out = self.text_encoder(prefix_shifted, self.classes)
        norm_out = out / out.norm(dim=-1, keepdim=True)
        return out

class CoCoopBaseline(object):
    def __init__(self, config, label_to_idx, 
        ...
        self.model = ExtendedTextPrefixModel(initial_prefix,
                                     self.text_encoder,
                                     [' '.join(c.split('_')) for c in self.seen_classes],
                                     prefix_dim=self.clip_model.token_embedding.embedding_dim,
                                     image_feat_dim=self.clip_model.visual.output_dim,
    ...
    def _train_epoch(self, loss, total_loss, train_loader, accum_iter, epoch):
        ...
        for i, (img, _, _, label, img_path) in enumerate(tqdm(train_loader)):
            ...
            text_features = self.model(image_features)
