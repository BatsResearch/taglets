...
class RankVPTBaseline(InitVPTBaseline):
    def __init__(
        self, 
        config, 
        label_to_idx,
    ...
    def _train_epoch(
        ...
        for i, (img, _, _, label, img_path) in enumerate(train_loader):
            # Get images from new prompt
            image_features_unseen = self.training_model(img, teacher)
            image_features_unseen = image_features_unseen / image_features_unseen.norm(dim=-1, keepdim=True)
            # get images from seen prompts
            with torch.no_grad():
