...
class CoopTeacherStudent(CoopPseudoBaseline):
    def __init__(
        self,
        config,
        label_to_idx,
    ...
    def test_predictions(self, data, standard_zsl=False, teacher=True):
        ...
        if teacher:
            ...
            log.info(f"Number of prompts: {len(self.teacher.classes)}")

            if standard_zsl:
                self.teacher.classes = self.unseen_classes
            else:
                self.teacher.classes = self.classes

            accelerator.wait_for_everyone()
            # Get prompts
            text_features = self.teacher(self.teacher.classes)
            ...
            log.info(f"Number of prompts: {len(self.student.classes)}")

            if standard_zsl:
                self.student.classes = self.unseen_classes
            else:
                self.student.classes = self.classes

            accelerator.wait_for_everyone()
            # Get prompts
            text_features = self.student(self.student.classes)
    ...
    def training_model(self, teacher=False):
        ...
        """
        if teacher:
            return self.teacher(self.teacher.classes)
        else:
            return self.student(self.student.classes,)
