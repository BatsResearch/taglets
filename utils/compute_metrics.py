...
def save_pseudo_labels(imgs, labs, config, iteration, teacher=False):
    ...
    if teacher:
        filename = f"pseudolabels/{config.DATASET_NAME}_{config.MODEL}_{config.VIS_ENCODER.replace('/', '')}_teacher_iter_{iteration}_pseudolabels_spl_{config.SPLIT_SEED}.pickle"
    else:
        filename = f"pseudolabels/{config.DATASET_NAME}_{config.MODEL}_{config.VIS_ENCODER.replace('/', '')}_student_iter_{iteration}_pseudolabels_spl_{config.SPLIT_SEED}.pickle"
