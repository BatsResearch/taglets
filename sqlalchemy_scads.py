...
class Relation(Base):
    ...
    edge = relationship("Edge",back_populates="relation")
...
class Edge(Base):
    ...
    info = Column(String)
    relation = relationship("Relation",back_populates="edge")
    def __repr__(self):
    ...
    with open(Path(cwd+'/sql_data/edges.csv')) as edge_file:
        for line in edge_file:
            if line.strip():
                ...
                print(key, URI, relation_id, start_node_id, end_node_id,weight)
                print(info)
